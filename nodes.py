    ...
    def rewrit(self, text, image, prompt_style, llm_model, max_retry, API_KEY, API_KEY_file, save_tokens, skip_rewrite):
        ...
        if API_KEY_file:
            ...
            with open(key_path, "r", encoding="utf-8") as f:
                content = f.read()
            api_key_ = content.strip()
        else:
            api_key_ = API_KEY
            
        if not api_key_.strip():
            raise EnvironmentError("API_KEY is not set!")
...
class TextEncodeQwenImageEditAdv:
    ...
    def encode(self, clip, prompt, negative_prompt, vae=None, image=None):
        ref_latent = None
        if image is None:
            images = []
        else:
...
class TextEncodeQwenImageEditPlusAdv:
    ...
    def encode(self, clip, prompt, negative_prompt, vae=None, image1=None, image2=None, image3=None):
        ref_latents = []
